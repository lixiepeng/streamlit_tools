...
def maximum_matching(train_text, decode_text,
                     entityRe=r'\[\@.*?\#.*?\*\](?!\#)',
                     recommendRe=r'\[\$.*?\#.*?\*\](?!\#)'):
    extracted_dict = {}
    max_length = 0
    ...
    for match in re.finditer(entityRe, decode_no_recommend):
        ...
        [entity, recognized_type] = recommend_entity.strip(
            '[@]*').rsplit('#', 1)
        decode_origin += entity
        entity_recognized_list += ["B-@-"+recognized_type] + \
    ...
    FMM_start = 0
    FMM_end = (FMM_start + max_length) if (FMM_start +
    ...
    while FMM_start < origin_length:
        ...
        # finish recognized span detection
            ...
            if word in extracted_dict:
                entity_recommend_list += ["B-$-"+extracted_dict[word]] + \
...
def merge_text_with_entity(origin_text, recognized_list, recommend_list):
    ...
    for idx in range(length):
        if combine_list[idx] == 0:
            if entity_string:
                new_string += "["+entity_source + \
                    entity_string + "#" + entity_type+"*]"
                entity_string = ""
                entity_type = "Error"
                entity_source = "Error"
            # print new_string
